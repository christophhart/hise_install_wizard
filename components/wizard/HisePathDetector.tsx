...
const detectionScripts: Record<Exclude<Platform, null>, string> = {
  ...
  macos: `h=$(which HISE 2>/dev/null);if [ -n "$h" ];then p=$(dirname "$(realpath "$h")");f=$([[ "$p" == *"Faust"* ]] && echo faust || echo nofaust);r=$(dirname "$(dirname "$(dirname "$(dirname "$(dirname "$(dirname "$(dirname "$(realpath "$h")")")")")")")");a=$(uname -m);[ -d "$r/.git" ] && echo "$r,valid,$f,$a" || echo "$r,invalid,$f,$a";else echo "not_found";fi`,
  linux: `h=$(which HISE 2>/dev/null);if [ -n "$h" ];then p=$(dirname "$(realpath "$h")");f=$([[ "$p" == *"Faust"* ]] && echo faust || echo nofaust);r=$(dirname "$(dirname "$(dirname "$(dirname "$(dirname "$(realpath "$h")")")")")");[ -d "$r/.git" ] && echo "$r,valid,$f" || echo "$r,invalid,$f";else echo "not_found";fi`,
};
